      ...
      <div class="p-4 sm:p-6">
        ...
        <div class="grid sm:grid-cols-3 sm:gap-4">
          ...
          <el-form-item :label="t('ASSIGNED_DOCTORS_COUNT')">
            <v-input :model-value="departmentData?.doctorCount" :readonly="true" />
          </el-form-item>
          <el-form-item :label="t('ASSIGNED_SERVICES_COUNT')">
            <v-input :model-value="departmentData?.serviceCount" :readonly="true" />
          </el-form-item>
        </div>
        <div class="grid sm:grid-cols-2 sm:gap-4">
          <el-form-item :label="t('CREATED_DATE')">
            <v-input :model-value="dayjs(departmentData?.creationDate).format('DD.MM.YYYY')" :readonly="true" />
          </el-form-item>
          <el-form-item :label="t('MODIFIED_DATE')">
            <v-input :model-value="departmentData?.modifiedDate ? dayjs(departmentData?.modifiedDate).format('DD.MM.YYYY'):''" :readonly="true" />
          </el-form-item>
        </div>
        <el-form-item :label="t('DEPARTMENT_ABOUT')" class="!mb-0">
          <v-input :model-value="departmentData?.description" type="textarea" :rows="4" :readonly="true" />
        </el-form-item>
      </div>
