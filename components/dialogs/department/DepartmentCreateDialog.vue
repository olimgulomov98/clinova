  ...
  >
    <template #header>
      <div class="p-4 sm:p-6 flex justify-between border-b border-solid border-gray-line">
        <v-form-title>{{ departmentId ? t('UPDATE_DEPARTMENT') : t('ADD_NEW_DEPARTMENT') }}</v-form-title>
    ...
    <el-form ref="formRef" label-position="top" :model="form" :rules="rules" @submit.prevent="submitForm(formRef)">
      <div class="p-4 sm:p-6">
        <el-form-item :label="t('DEPARTMENT_NAME')" prop="name">
          <el-input v-model="form.name" class="form_input" :placeholder="t('ENTER_DEPARTMENT_NAME')" />
        </el-form-item>
        <el-form-item :label="t('DEPARTMENT_ABOUT')" prop="description" class="!mb-0">
          <el-input v-model="form.description" class="form_input" type="textarea" :rows="4" />
        </el-form-item>
      </div>
...
import type { IDepartmentCreate } from "~/types/department/index.type";
const { t } = useI18n();
...
async function createDepartment() {
  ...
  (<Axios>$axios)
    [method](url, { ...form, id })
    .then((res) => {
      notificationShower("success", id ? t('DEPARTMENT_UPDATE_SUCCESS') : t('DEPARTMENT_CREATED_SUCCESS'));
      emit("close");
      emit("getData");
    })
    .finally(() => {
...
const getDepartmentById = async () => {
  (<Axios>$axios).get(`/api/department/summary/${props.departmentId}`).then((res) => {
