  ...
  <div class="page-container">
    <div v-if="isMounted" class="demo-collapse-position">
      ...
      <el-collapse v-model="activeNames" :expand-icon-position="'left'">
        <el-collapse-item
          v-for="(collapse, index) in diagnosis?.filter(
        ...
        >
          <div
            class="bg-[#F0F0F0] p-4 rounded-[8px] text-[14px] text-[#757575]"
          >
            {{ collapse?.text }}
          </div>
        </el-collapse-item>
        ...
        >
          <div>
            <VTable :table-data="visits?.images" with-out-search>
              <template #columns>
...
const getDataDiagnosisDetail = async () => {
  (<Axios>$axios)
    .post("/api/diagnosis/list", {
      visitId: Number(route.params?.id),
...
onMounted(() => {
  ...
  activeNames.value = [222, 45, 3, 102, 101, "index1", "index-download2"];
});

const downloadPrintInvoice = async () => {
  try {
    const response = await (<Axios>$axios).get(
      `/api/pdf/visit/${visits.value?.id}`,
      {
        responseType: "blob",
      }
    );
